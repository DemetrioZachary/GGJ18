...
public class PlayerController : MonoBehaviour {
    ...
    private void Fire() {
        ...
        if (fireTime<=0 && prevState.Buttons.RightShoulder == ButtonState.Released && state.Buttons.RightShoulder == ButtonState.Pressed) {
            fireTime = fireDelay;
            float xValue = state.ThumbSticks.Left.X;
            float yValue = state.ThumbSticks.Left.Y;
            if (Mathf.Abs(xValue) < 0.01 && Mathf.Abs(yValue) < 0.01) { xValue = 1; }
            Projectile projectile = Instantiate(projectilePrefab, transform.position,Quaternion.Euler(0,0,Mathf.Atan2(yValue,xValue)*Mathf.Rad2Deg)) as Projectile;
    ...
    public void HandleHit(GameManager.Types impactType) {
        if (impactType != shield) {
            // TODO
        }
    }
